...
static gboolean
handle_client(GIOChannel *source, GIOCondition condition, LXTermWindow *lxtermwin) {
	if (condition & G_IO_IN) accept_client(source, lxtermwin);
...
handle_request(GIOChannel *gio, GIOCondition condition, ClientInfo *info) {
	...
	if (ret == G_IO_STATUS_ERROR) g_warning("Error reading socket: %s\n", err->message);
	...
	if (len > 0) {
		...
		lxtermc_args(argc, argv, &arguments);
		g_free(argv);

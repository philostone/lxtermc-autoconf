...
static gboolean
preferences_dialog_background_color_set_event(GtkWidget *widget, Setting *setting)
{
...
static gboolean
preferences_dialog_foreground_color_set_event(GtkWidget *widget, Setting *setting)
{
...
static gboolean
preferences_dialog_palette_color_set_event(GtkColorButton *widget, Setting *setting)
{
	for (int i = 0;i < 16; i++) {
...
static gboolean
preferences_dialog_palette_preset_changed_event(GtkComboBox *widget, Setting *setting)
{
	gint active;
	GtkWidget *w;

	active = gtk_combo_box_get_active(widget);
	if (g_strcmp0(color_presets[active].name, "Custom") == 0) return FALSE;
...
static void
preferences_dialog_allow_bold_toggled_event(GtkToggleButton *widget, Setting *setting)
{
...
static void
preferences_dialog_tab_position_changed_event(GtkComboBox *widget, Setting *setting)
{
...
static void
preferences_dialog_int_value_changed_event(GtkSpinButton *widget, gint *value)
{
	*value = gtk_spin_button_get_value_as_int(widget);
}

/* Convert the user preference on tab position, expressed as a string, to the internal
 * representation. These have to match the order in the .glade file. */
gint
terminal_tab_get_position_id(gchar *position)
{
...
static void
preferences_dialog_generic_toggled_event(GtkToggleButton *widget, gboolean *s)
{
...
static gboolean
preferences_dialog_generic_focus_out_event(GtkWidget *widget, GdkEventFocus *event, gchar **s)
{
	g_free(*s);
...
static gboolean
preferences_dialog_shortcut_key_press_event(GtkWidget *widget, GdkEventKey *ekey, gchar **s)
{
	...
	for (sib = gtk_container_get_children(GTK_CONTAINER(gtk_widget_get_parent(widget)));
		sib; sib = sib->next) {
		if (GTK_IS_ENTRY(sib->data) && GTK_WIDGET(sib->data)
			!= widget && !g_strcmp0(lbl, gtk_entry_get_text(GTK_ENTRY(sib->data)))) {
			goto free_lbl;
		}
	}

	/* Make sure accelerator is valid. */
	if (!gtk_accelerator_valid(key, mod)) goto free_lbl;
...
static void
accel_set_label(const gchar *name, GtkWidget *w)
{
	guint key;
...
terminal_preferences_dialog(GtkAction *action, LXTerminal *terminal)
{
	...
	g_signal_connect(G_OBJECT(w), "font-set",
		G_CALLBACK(preferences_dialog_font_set_event), setting);
	...
	for (int i = 0;; i++) {
		...
		if (g_strcmp0(color_presets[i].name, "Custom") == 0) {
			if (preset_is_set == FALSE) gtk_combo_box_set_active(w2, i);
			preset_custom_id = i;
			break;
		}
	}
	...
	gtk_combo_box_set_active(GTK_COMBO_BOX(w),
		terminal_tab_get_position_id(setting->tab_position));
	...
	g_signal_connect(G_OBJECT(w), "focus-out-event",
		G_CALLBACK(preferences_dialog_generic_focus_out_event),

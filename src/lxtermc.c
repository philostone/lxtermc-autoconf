...
/* philostone - assume vte version 0.72.2 or above ... */
/* philostone - assume gtk version 4.10.5 or above ... */
/* philostone - assume glib version 2.76.4 or above ... */
...
static void
term_tab_set_position(GtkWidget *notebook, gint tab_position)
{
	switch (tab_position) {
	case 0:
...
static gchar *
term_get_current_dir(LXTerminal *terminal)
{
	gchar *proc_cwd = NULL;
...
static void
term_initialize_switch_tab_accelerator(Term *term)
{
	if ((term->index + 1) < 10) {
...
/* update <ALT>n status. */
void
term_update_alt(LXTerminal *terminal)
{
	guint i;
	Term * term;
...
static gboolean
term_switch_tab_accelerator(Term *term)
{
 ...
 * Open a new window. */
static void
term_new_window_activate_event(GtkAction *action, LXTerminal *terminal)
{
 ...
 * Open a new tab. */
static void
term_new_tab_activate_event(GtkAction *action, LXTerminal *terminal)
{
	term_new_tab(terminal, NULL);
}

static void
term_set_geometry_hints(Term *term, GdkGeometry *geometry)
{
	...
	term_get_border(term, &termBorder);
...
static void
term_save_size(LXTerminal *terminal)
{
...
static void
term_new_tab(LXTerminal *terminal, const gchar *label)
{
	Term *term;
	...
	if (terminal->login_shell) {
		...
		exec[0] = g_strdup(term_get_preferred_shell());
		char *shellname = g_path_get_basename(exec[0]);
		exec[1] = g_strdup_printf("-%s", shellname);
		g_free(shellname);
		exec[2] = NULL;
		term = term_new(terminal, label, proc_cwd, NULL, exec);
	} else {
		term = term_new(terminal, label, proc_cwd, NULL, NULL);
	}
	g_free(proc_cwd);
	...
	if (term->index > 0) {
		term_save_size(terminal);
		gtk_notebook_set_show_tabs(GTK_NOTEBOOK(term->parent->notebook), TRUE);
	}

	/* Disable Alt-n switch tabs or not. */
	term_update_alt(terminal);
}

static void
term_vte_size_allocate_event(GtkWidget *widget, GtkAllocation *allocation, Term *term)
{
	GdkGeometry geometry;
	term_set_geometry_hints(term, &geometry);
	g_signal_handlers_disconnect_by_func(widget, term_vte_size_allocate_event, term);
}
...
static void
term_close_window_activate_event(GtkAction *action, LXTerminal * terminal)
{
	if (!term_close_window_confirmation_dialog(terminal)) return;
...
static void
term_name_tab_response_event(GtkWidget *dialog, gint response, Term *term)
{
...
static void
term_name_tab_activate_event(GtkAction *action, LXTerminal *terminal)
{
	Term *term;
	gint current = gtk_notebook_get_current_page(GTK_NOTEBOOK(terminal->notebook));
	if (current == -1 || terminal->terms->len <= 0) return;

	/* Search for the Term structure corresponding to the current tab. */
	guint i;
	for (i = 0; i < terminal->terms->len; i++) {
		term = g_ptr_array_index(terminal->terms, i);
		if (term->index == current) break;
	}
...
static void
term_previous_tab_activate_event(GtkAction *action, LXTerminal *terminal)
{
	/* Cycle through tabs. */
...
static void
term_next_tab_activate_event(GtkAction *action, LXTerminal *terminal)
{
	/* Cycle through tabs. */
	if (gtk_notebook_get_current_page(GTK_NOTEBOOK(terminal->notebook))
...
static void
term_move_tab_execute(LXTerminal *terminal, gint direction)
{
	...
	if (target_page_number < 0 || (guint) target_page_number >= terminal->terms->len) return;
	...
	gtk_notebook_reorder_child(notebook,
		gtk_notebook_get_nth_page(notebook, current_page_number), target_page_number);
...
static void
term_move_tab_left_activate_event(GtkAction *action, LXTerminal *terminal)
{
...
static void
term_move_tab_right_activate_event(GtkAction *action, LXTerminal *terminal)
{
	term_move_tab_execute(terminal, 1);
}
...
static void
term_zoom(LXTerminal *terminal)
{
	Term *term;
	guint i;
	term_save_size(terminal);

	if (terminal->terms->len <= 0) return;
...
static gboolean
term_zoom_in_activate_event(GtkAction *action, LXTerminal *terminal)
{
	terminal->scale += 0.1;
	term_zoom(terminal);
	return FALSE;
...
static gboolean
term_zoom_out_activate_event(GtkAction *action, LXTerminal *terminal)
{
	if (terminal->scale < 0.3) return FALSE;
	terminal->scale -= 0.1;
	term_zoom(terminal);
	return FALSE;
}
...
static gboolean
term_zoom_reset_activate_event(GtkAction *action, LXTerminal *terminal)
{
	terminal->scale = 1.0;
	term_zoom(terminal);
	return FALSE;
...
static void
term_about_activate_event(GtkAction *action, LXTerminal *terminal)
{
	const gchar * authors[] = {
...
static void
term_switch_page_event(GtkNotebook *notebook, GtkWidget *page, guint num, LXTerminal *terminal)
{
	if (terminal->terms->len > num) {
		...
		if (titaux && g_str_has_prefix(titaux,"* ")) titaux+=2;
		...
		g_signal_connect(notebook, "size-allocate", G_CALLBACK(term_vte_size_allocate_event), term);
	}
}

/* Handler for "window-title-changed" signal on a Term. */
static void
term_window_title_changed_event(GtkWidget *vte, Term *term)
{
...
static gboolean
term_close_window_confirmation_event(GtkWidget *widget, GdkEventButton *event, LXTerminal *terminal)
{
	return !term_close_window_confirmation_dialog(terminal);
}
...
term_close_window_confirmation_dialog(LXTerminal *terminal)
{
	if (!terminal->setting->disable_confirm && terminal->terms->len > 1) {
		...
		gtk_widget_destroy(dialog);
		if (result != GTK_RESPONSE_YES) return FALSE;
	}
	return TRUE;
}
...
static void
term_window_exit(LXTerminal *terminal, GObject *where_the_object_was)
{
...
static gboolean
term_tab_button_press_event(GtkWidget *widget, GdkEventButton *event, Term *term)
{
	if (event->button == 2) {
		/* Middle click closes the tab. */
		term_close_button_event(NULL, term);
		return TRUE;
	}
	return FALSE;
}
...
static void
term_show_popup_menu(VteTerminal *vte, GdkEventButton *event, Term *term)
{
	/* Generate popup menu. */
	...
	term->matched_url = term_get_match_at(vte, term, event);
...
static void
term_vte_cursor_moved_event(VteTerminal *vte, Term *term)
{
	LXTerminal *terminal = term->parent;
	...
	if (activeterm != term) {
		if (term->user_specified_label) {
			...
			gtk_label_set_markup(GTK_LABEL(term->label), g_strconcat("<b>* ",
				vte_terminal_get_window_title(VTE_TERMINAL(vte)), "</b>", NULL));
		}
	}
}
...
static void
term_vte_visual_bell(VteTerminal *vte, Term *term)
{
...
static gboolean
term_vte_button_press_event(VteTerminal *vte, GdkEventButton *event, Term *term)
{
	...
	if (event->button == 3) {
		...
		if (event->state & GDK_CONTROL_MASK) {
			term_show_popup_menu(vte, event, term);
			return TRUE;
		} else {
		...
		/* Control left click. */
		gchar *match = term_get_match_at(vte, term, event);
		if (match != NULL) {
...
static gboolean
term_vte_button_release_event(VteTerminal *vte, GdkEventButton *event, Term *term)
{
...
static void
term_vte_commit(VteTerminal *vte, gchar *text, guint size, Term *term)
{
	term->open_menu_on_button_release = FALSE;
}
...
//		vte_terminal_set_background_saturation(VTE_TERMINAL(term->vte), 1 - ((double) setting->background_alpha / 65535));
	...
	if (setting->visual_bell) {
		g_signal_connect(G_OBJECT(term->vte), "bell", G_CALLBACK(term_vte_visual_bell), term);
	} else {
		g_signal_handlers_disconnect_by_func(G_OBJECT(term->vte), term_vte_visual_bell, term);
	}
}

/* Create a new terminal. */
static Term *
term_new(LXTerminal *terminal, const gchar *label, const gchar *pwd, gchar **env,  gchar **exec)
{
	...
	/* Connect signals. */
	g_signal_connect(G_OBJECT(term->tab), "button-press-event", G_CALLBACK(term_tab_button_press_event), term);
	g_signal_connect(G_OBJECT(term->close_button), "clicked", G_CALLBACK(term_close_button_event), term);
	g_signal_connect(G_OBJECT(term->vte), "button-press-event", G_CALLBACK(term_vte_button_press_event), term);
	g_signal_connect(G_OBJECT(term->vte), "button-release-event", G_CALLBACK(term_vte_button_release_event), term);
	g_signal_connect(G_OBJECT(term->vte), "commit", G_CALLBACK(term_vte_commit), term);
	term->exit_handler_id = g_signal_connect(G_OBJECT(term->vte), "child-exited", G_CALLBACK(term_child_exited_event), term);
	g_signal_connect(G_OBJECT(term->vte), "cursor-moved", G_CALLBACK(term_vte_cursor_moved_event), term);
	g_signal_connect(G_OBJECT(term->vte), "window-title-changed", G_CALLBACK(term_window_title_changed_event), term);

	/* Show the widget and return. */
	gtk_widget_show_all(term->box);

	/* Apply user preferences. */
	term_settings_apply_to_term(terminal, term);
	return term;
}
...
static void
term_menubar_initialize(LXTerminal *terminal)
{
...
static void
term_menu_accelerator_update(LXTerminal *terminal)
{
...
gboolean
lxtermc_args(gint argc, gchar **argv, CommandArguments *arguments)
{
	...
	while (argc > 1) {
		 ...
		 * The <rest of arguments> behavior is demanded by distros
		 * who insist on this xterm feature. */
			...
			if (bitmask & WidthValue) arguments->geometry_columns = width;
			if (bitmask & HeightValue) arguments->geometry_rows = height;
			if (bitmask & XValue) arguments->geometry_xoff = xoff;
			if (bitmask & YValue) arguments->geometry_yoff = yoff;
		...
		} else if (((strcmp(argument, "--title") == 0) || (strcmp(argument, "-t") == 0) || (strcmp(argument, "-T") == 0)) && (argc > 1)) {
	...
	if (arguments->login_shell == TRUE) {
		const gchar *shell = term_get_preferred_shell();
...
// TODO GFile * instead of filenames !!!!!
	...
	g_signal_connect_swapped(G_OBJECT(terminal->window), "composited-changed",
		G_CALLBACK(term_settings_apply), terminal);
	g_signal_connect(G_OBJECT(terminal->notebook), "switch-page",
		G_CALLBACK(term_switch_page_event), terminal);
	g_signal_connect(G_OBJECT(terminal->window), "delete-event",
		G_CALLBACK(term_close_window_confirmation_event), terminal);

	/* Create the first terminal. */
	gchar *local_working_directory = NULL;
	if (arguments->working_directory == NULL) local_working_directory = g_get_current_dir();
	Term *term = term_new(
		terminal,
...
term_settings_apply(LXTerminal *terminal)
{
	...
	guint i;
	for (i = 0; i < terminal->terms->len; i++) {
		term_settings_apply_to_term(terminal, g_ptr_array_index(terminal->terms, i));
	}
	...
	/* Wait for its size to be allocated, then set its geometry */
	g_signal_connect(term->vte, "size-allocate", G_CALLBACK(term_vte_size_allocate_event), term);
}
...
void
term_update_menu_shortcuts(Setting *setting)
{
	guint key;
	GdkModifierType mods;
...
void
term_initialize_menu_shortcuts(Setting *setting)
{
...
main(gint argc, gchar **argv)
{
	...
	CommandArguments arguments;
	if (!lxtermc_args(argc, argv, &arguments)) return 0;

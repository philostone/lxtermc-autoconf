...
typedef struct _command_arguments {
	char *executable;			/* Value of argv[0]; points into arg vector */
	gchar **command;			/* Value of -e, --command; mem malloc by glib */
	gchar *config;				/* Value of -c, --config; points into arg vector */
	int geometry_bitmask;
	unsigned int geometry_columns;		/* Value of --geometry */
	unsigned int geometry_rows;
	int geometry_xoff;
	int geometry_yoff;
	char *title;				/* Value of -t, -T, --title; points into arg vector */
	char *tabs;				/* Value of --tab; points into arg vector */
	char *working_directory;		/* Value of --working-directory; points into arg vector */
	gboolean login_shell;			/* Terminal will spawn login shells */
	gboolean no_remote;
} CommandArguments;
